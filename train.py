...
def train(args):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for i, (inputs, labels) in enumerate(dataloaders['test']):
                ...
                #calculation
                _, pre_labels = torch.max(output.data, 1)
                precision, recall, f1, accuracy = evaluation(labels, pre_labels)
            ...
            test_epoch_accuracy = sum(test_accuracy) / test_iteration
            ...
            #write accuracy
            writer.add_scalar('Test/total_accuracy', float(sum(test_epoch_accuracy) / 5), epoch)
            writer.add_scalars('Test/accuracy', {'daisy':test_epoch_accuracy[0], 'dandelion':test_epoch_accuracy[1], 'rose':test_epoch_accuracy[2], 'sunflower':test_epoch_accuracy[3], 'tulip':test_epoch_accuracy[4]}, epoch)
            #write recall
            ...
            plt_test_accuracy.append(test_epoch_accuracy)
    ...
    evaluations = [plt_train_losses, plt_test_precision, plt_test_accuracy, plt_test_recall, plt_test_F1, plt_test_losses]
    ...
    model_name = args.network + '_' + 'newfc_' + str(args.new_fc) + '.pth'
...
def evaluation(gt, pre):
    ...
    return precision_classes, recall_classes, f1_classes, accuracy_classes
...
def draw_curces(matrix, epoch, path):
    ...
    #train loss
    plt.plot(x, np.array(matrix[0]), 'r', label = 'training loss')
    ...
    plt.legend(loc='lower right')
    plt.savefig(os.path.join(path, './test_accuracy.png'))
    plt.cla()

    #test recall
    plt.plot(x, np.array(matrix[3])[:, 0], 'r', label = 'daisy')
    plt.plot(x, np.array(matrix[3])[:, 1], 'g', label = 'dandelion')
    plt.plot(x, np.array(matrix[3])[:, 2], 'b', label = 'sunflower')
    plt.plot(x, np.array(matrix[3])[:, 3], 'y', label = 'rose')
    plt.plot(x, np.array(matrix[3])[:, 4], 'c', label = 'tulip')
    plt.plot(x, np.sum(np.array(matrix[3]), axis = 1) / 5, 'k', label = 'average')
    ...
    #test loss
    plt.plot(x, np.array(matrix[5]), 'r', label = 'Test loss')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--network', type=str, default = 'mobilenet_v2', help='The CNN model architecture for training, option: vgg16, densenet161, resnet18, mobilenet_v2')
